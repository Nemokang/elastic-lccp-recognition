...
//    viewer->setBackgroundColor(0,0,0);
    ...
    while(!pangolin::ShouldQuit() && !((!logReader->hasMore()) && quiet) && !(eFusion->getTick() == end && quiet))
    {
        ...
        gui->preCall();
        /********* 放在这里全局模型会显示在小窗口内　, 放在preCall之前，不会显示　***********************************************************************************/
                //gui->displayImg("ModelImg", eFusion->getIndexMap().vertexTex());
                //gui->displayImg("Model", eFusion->getIndexMap().normalTex());
        /********************************************************************************************/
...
/********************************************************************************************/
        ...
        for(std::map<std::string, GPUTexture*>::const_iterator it = eFusion->getTextures().begin(); it != eFusion->getTextures().end(); ++it)
        {
            if(it->first.compare(GPUTexture::RGB)==0)
            {
                gui->displayImg("ModelImg", it->second);
            }
