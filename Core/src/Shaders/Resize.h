...
class Resize
{
    public:
        ...
        void normal_noDownSampling(GPUTexture * source, Img<Eigen::Vector4f> & dest );//-------------------new add
        void time(GPUTexture * source, Img<unsigned short> & dest);
        void vertex_noDownSampling(GPUTexture * source, Img<Eigen::Vector4f> & dest);//-------------------new add------------

        GPUTexture imageTexture;
        GPUTexture vertexTexture;
        GPUTexture normalTexture;//------------new add
        GPUTexture timeTexture;
        GPUTexture rawVertexTexture;//------------new add----------
        ...
        std::shared_ptr<Shader> normalProgram;//--------------------------------new add
        pangolin::GlRenderBuffer normalRenderBuffer;//--------------------------------new add
        pangolin::GlFramebuffer normalFrameBuffer;//--------------------------------new add

        std::shared_ptr<Shader> timeProgram;
        pangolin::GlRenderBuffer timeRenderBuffer;
        pangolin::GlFramebuffer timeFrameBuffer;

        std::shared_ptr<Shader> rawVertexProgram;//--------------------------------new add--------------
        pangolin::GlRenderBuffer rawVertexRenderBuffer;//--------------------------------new add------------
        pangolin::GlFramebuffer rawVertexFrameBuffer;//--------------------------------new add------------
};
