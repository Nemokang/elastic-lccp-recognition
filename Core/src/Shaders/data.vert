...
uniform sampler2D cSampler;//0
uniform sampler2D drSampler;//1----原始深度图----DEPTH_METRIC
uniform sampler2D drfSampler;//2－－－－－滤波后的深度图---DEPTH_METRIC_FILTERED
uniform usampler2D indexSampler;//3
uniform sampler2D vertConfSampler;//4
uniform sampler2D colorTimeSampler;//5
uniform sampler2D normRadSampler;//6
uniform sampler2D lSampler;//----------------------------------7

uniform vec4 cam; //cx, cy, 1/fx, 1/fy
uniform float cols;//(float)Resolution::getInstance().cols())
uniform float rows;//(float)Resolution::getInstance().rows())
uniform float scale;//(float)IndexMap::FACTOR)
uniform float texDim;//(float)TEXTURE_DIMENSION)
uniform mat4 pose;
uniform float maxDepth;
uniform float time;//(float)time
...
void main()
{
   ...
   ///////////////////////////////////////////////////////////////////////////////////////////////////////new add
  //  vec4 vColorTemp=textureLod(lSampler, texcoord.xy, 0.0);
  //  vColor.y = encodeColor(vColorTemp.xyz);
   vColor.y = float(textureLod(lSampler, texcoord.xy, 0.0));
