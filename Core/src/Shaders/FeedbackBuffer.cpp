...
//vertex_feedback.vert  vertex_feedback.geom
                             ...
                             const float depthCutoff)
{
    ...
    program->setUniform(Uniform("lSampler", 2));//-------------------------------------------new add
    ...
    glBindTexture(GL_TEXTURE_2D, label_rgb->tid);
    //-------end --------------------------------------------------new add---------------

    glDrawArrays(GL_POINTS, 0, Resolution::getInstance().numPixels());//应用程序通过绘制命令启动管线（如glDrawElements( GL_POINTS/LINES/TRIANGLES/PATCHES, )）；
