                                ...
                                const Eigen::Matrix4f & modelPose)
{
    ...
    cudaGraphicsMapResources(1, &predictedVertices->cudaRes);//映射图形资源以供CUDA访问，映射&predictedVertices->cudaRes中的１个资源
    cudaGraphicsSubResourceGetMappedArray(&textPtr, predictedVertices->cudaRes, 0, 0);//获得已映射图形资源的子资源对应的数组，将predictedVertices->cudaRes的子资源数组返回给&textPtr
    cudaMemcpyFromArray(vmaps_tmp.ptr(), textPtr, 0, 0, vmaps_tmp.sizeBytes(), cudaMemcpyDeviceToDevice);//目标地址，源，起始位置为左上角位置（０，０），传输尺寸，传输方向
    cudaGraphicsUnmapResources(1, &predictedVertices->cudaRes);//解除映射图形资源
                                                ...
                                                const bool & so3)
{
    ...
    Eigen::Matrix<double, 3, 3, Eigen::RowMajor> resultR = Eigen::Matrix<double, 3, 3, Eigen::RowMajor>::Identity();//用单位矩阵初始化
